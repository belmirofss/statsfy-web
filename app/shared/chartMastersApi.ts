...
const getMostStreamedTracks = async (
  browser: any,
  invalidate: boolean = false
  ...
  try {
    ...
    if (!invalidate) {
      return await getMostStreamedTracks(browser, true);
    } else {
...
const getMostStreamedArtists = async (
  browser: any,
  invalidate: boolean = false
  ...
  try {
    ...
    if (!invalidate) {
      return await getMostStreamedArtists(browser, true);
    } else {
      return Promise.reject(err);
    }
  }
...
export const getChartMastersMostStreamed = async () => {
  ...
  const mostStreamedTracks = await getMostStreamedTracks(browser);
  const mostStreamedArtists = await getMostStreamedArtists(browser);

  await browser.close();

  return { mostStreamedTracks, mostStreamedArtists };
};
